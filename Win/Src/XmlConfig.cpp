...
int		XmlConfig::LoadConfig(const char *filename, XML_CONFIG_T *pConfig)
{
	...
	if (!m_DocR.LoadFile(filename))
	{
		...
		char szTime[64] = { 0 };
		time_t tt = time(NULL);
		unsigned int randValue = tt & 0xFFFFF;
		...
		strcpy(xmlConfig.serverDomain, "3402000000");
		strcpy(xmlConfig.serverIP, "demo.easygbs.com");
		xmlConfig.serverSipPort = 30310;
		...
		sprintf(xmlConfig.deviceName, "EasyGBD-win-%d", randValue);
		strcpy(xmlConfig.sourcePath, "rtsp://");

		strcpy(xmlConfig.channels.cid, "34020000001310000001");
	...
	if (NULL != pServerConfigXML)
	{
		...
		pE = pServerConfigXML->FirstChildElement("serverDomain");
		if (pE && pE->GetText())		strcpy(xmlConfig.serverDomain, pE->GetText());
		...
		TiXmlElement* pChannelConfigXML = pServerConfigXML->FirstChild("Channels")->ToElement();
		if (pChannelConfigXML != NULL)
		{
			pE = pChannelConfigXML->FirstChildElement("cid");
			if (pE && pE->GetText()) {
				strcpy(xmlConfig.channels.cid, pE->GetText());
			}
		}
...
void	XmlConfig::SaveConfig(const char *filename, XML_CONFIG_T *pConfig)
{
	...
	AddElement((char*)"serverDomain",		xmlConfig.serverDomain,			pRootElm);
	...
	TiXmlElement* pChannelElm = NULL;
	pChannelElm = new TiXmlElement("Channels");
	AddElement((char*)"cid", xmlConfig.channels.cid, pChannelElm);
	pRootElm->InsertEndChild(*pChannelElm);
